...
public class KartView extends JFrame implements ActionListener {

    private int genislik = 910;
    private int yukseklik = 700;
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnKaydet){
            hastaEkle(txtTC.getText(), txtAdSoyad.getText(), txtTelefon.getText(), txtAdres.getText(), txtDogumTarihi.getText(),
                        txtKan.getText(), txtCinsiyet.getText(), txtMedeni.getText(), imgProfilResmi.);
        }
    }


    static void hastaEkle(String tc, String adSoyad, String telefon, String adres, String dogumTarihi, String kan, String cinsiyet, String medeniHal, String profilResmiPath) {
        if (anahtar.getText().equals("") || karsilik.getText().equals("")) {
            anahtar.requestFocus();
        } else {
            for (int i = 0; i < tm.getRowCount(); i++) {
                if (tm.getValueAt(i, 0).toString().equals(tc)) {
                    return;
                }
            }
            tm.insertRow(table.getRowCount(), new Object[]{key, pair});
        }
    }
}
